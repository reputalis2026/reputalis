...
- **Campos:** `name` (nombre completo o visible), `alias` (identificador corto histórico; se conserva en BD y en lógica pública donde aplique, pero ya no se edita ni muestra en Create/Edit), `photo` (ruta de imagen subida, disco `public`, directorio `employees`), `position` (puesto visible bajo el nombre en tarjetas), `is_active` (estado funcional del empleado). Se usa como **catálogo de empleados por cliente** para asociar encuestas a un empleado concreto (`employee_id` en CsatSurvey).
- **Relaciones:** `client()` BelongsTo Client; `nfcTokens()` HasOne NfcToken.
- **Regla NFC:** cada `Employee` tiene **exactamente 1 token lógico NFC** (aunque se puedan imprimir/cargar varias tarjetas con el mismo token). El valor `nfctokens.token` es estable y permanente: no se regenera ni sustituye al editar, activar o desactivar; solo se crea si el empleado no tenía ninguno por ausencia previa. La URL pública usa ese token: `/survey/nfc/{token}`.
- **Estado funcional:** `Employee::booted()` sincroniza cambios de `employees.is_active` hacia `nfctokens.is_active`. Si un empleado se desactiva, su token queda inactivo; si se reactiva, se reactiva el token existente. Si al crear/editar falta token, `CreateEmployee::afterCreate()` / `EditEmployee::afterSave()` lo crean con el mismo estado lógico del empleado (`is_active` true/false), sin activar por error empleados inactivos.
...
- **Borrado de empleados (importante PostgreSQL):** al eliminar un `Employee`, el `NfcToken` asociado se elimina por FK `ON DELETE CASCADE`. En panel, solo se permite borrar empleados inactivos; empleados activos no muestran acción de borrado y `EmployeeResource::canDelete()` devuelve `false`.
...
- **EmployeeResource:** CRUD de empleados (formulario con `name`, `photo`, `position`, `is_active`; el campo `alias` se conserva en BD/modelo pero se ocultó de Create/Edit). **No aparece en el menú** (`shouldRegisterNavigation` false); la gestión se hace desde **Cliente → Empleados**. SuperAdmin: ver/crear/editar cualquier empleado y borrar solo inactivos. Distribuidor: ver/crear/editar empleados de clientes que él creó y borrar solo inactivos. Cliente: solo **ver** empleados de su `ownedClient` (desde el ítem de menú “Empleados”). Create acepta `?client_id=` en la URL y redirige tras guardar a ClientResource empleados del cliente. Create/Edit no muestran breadcrumbs, no muestran Token NFC ni “Copiar enlace”, y Create no muestra “crear y crear otro”.
- **NfcTokenResource:** **oculto** (sin menú global y sin CRUD accesible). La gestión del token NFC se mantiene interna/automática desde el flujo de empleados: no se regenera ni sustituye el token existente; activar/desactivar empleado solo sincroniza `nfctokens.is_active`.
...
- **Encuesta pública:**
  ...
  - `GET /survey/nfc/{token}` → resuelve token → cliente + empleado; bloquea token inactivo, empleado inactivo o desajuste cliente/empleado; si es válido renderiza la encuesta CSAT preasignando `employee_id` vía `employee_code`.
...
- **App\Http\Controllers\SurveyController (web):** también incluye `showNfc(token)` para `/survey/nfc/{token}`. Resuelve `client` + `employee`, bloquea si el token está inactivo, si el empleado está inactivo o si cliente/empleado no corresponden, y renderiza `survey.blade.php` con `showNfcDemo=false` y `employee_code` preasignado (para que el API cree `CsatSurvey` con `employee_id`). Si el empleado está inactivo, devuelve `survey-nfc-invalid.blade.php` con mensaje móvil específico y botón “Cerrar”.
- **App\Http\Controllers\Api\SurveyController:** `store` para crear encuesta; valida con `StoreSurveyRequest`; busca cliente activo por `client_code`, empleado opcional activo por `employee_code` (campo `name` del empleado). Si `employee_code` corresponde a un empleado inactivo, responde 422 y no asocia `employee_id`. Si el score **no** está en `client_improvement_configs.positive_scores`: si viene `improvement_option_id`, valida que la opción pertenezca al cliente (vía ClientImprovementOption → clientImprovementConfig → client_id) y guarda en `csat_surveys.improvement_option_id`; opcionalmente rellena `improvementreason_id` desde `improvement_reason_code`. Si solo viene `improvement_reason_code`, valida motivo activo y guarda `improvementreason_id`.
...
- **Empleados por cliente:** Catálogo de empleados (name, alias, photo, position, is_active) por cliente para encuestas asociadas a empleado. **ClientResource → Empleados:** subpágina con tarjetas y selector tipo pestaña **“Empleados activos”** / **“Empleados inactivos”**; solo se carga el grupo seleccionado. La tarjeta muestra foto/placeholder, nombre y `position` bajo el nombre si existe. SuperAdmin y Distribuidor pueden crear y editar; el borrado solo se permite si `is_active = false` (botón oculto en activos, `deleteEmployee()` y `EmployeeResource::canDelete()` lo bloquean en backend; `canDeleteAny()` queda deshabilitado). Distribuidor solo en clientes que él creó. Rol **cliente** usa la página **ClientEmpleados** (menú “Empleados”) en solo lectura, no ClientResource; además `ClientResource → Empleados` bloquea acceso por URL para cliente (`403`). EmployeeResource no está en el menú; create/edit se abren desde la subpágina (create con `?client_id=`; redirect tras guardar a Empleados del cliente). Create/Edit no muestran breadcrumbs, alias, Token NFC ni copiar enlace; Create solo tiene Crear/Cancelar. **Permisos recurso:** `EmployeeResource::canAccess()` debe permitir crear/ver rutas del recurso a quien `canCreate()`/`canViewAny()` aplique (Filament llama `canAccess` al montar paginas del recurso). **Alta:** con `client_id` en query, autorizacion alineada a `ClientResource::canEdit` en `CreateEmployee`.
- **NFC por empleado:** la acción visible de panel para copiar enlace vive en `ClientResource → Empleados`, por tarjeta, y copia `/survey/nfc/{token}` con `navigator.clipboard` y fallback compatible. El token de un empleado es estable: no se regenera ni sustituye al editar, activar o desactivar. `employees.is_active` sincroniza `nfctokens.is_active`; un empleado inactivo bloquea la encuesta NFC y muestra una vista móvil amigable con el mensaje “El empleado asociado a esta encuesta ya no se encuentra activo. Muchas gracias.” y botón “Cerrar”. La API `POST /api/surveys/create` rechaza `employee_code` de empleados inactivos con 422 y no asocia `employee_id`.
