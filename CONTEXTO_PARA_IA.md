...
- **Staff vs cliente al abrir un cliente:** Superadmin/distribuidor → hub `ClientResource` página `hub` (`/{record}/inicio`) con 5 cards. Propietario → dashboard `/{record}`. Barra flotante `client-panel-preview-bar` para volver al hub. Sidebar del cliente **sí** en Dashboard/Certificados/Informes (`isClientExperienceRoute`); **no** en Ficha, Encuesta, Empleados, Llamadas ni en editar/crear empleado o editar ficha (`hidesStaffSidebar` / clase `reputalis-staff-fullscreen`). Preview de sesión: `reputalis.client_panel_preview_id` + middleware `SyncClientPanelPreview`. SPA: `spaUrlExceptions` en empleados/ficha/edit. Cancelar edición usa URL a ficha/empleados, no `history.back()`.
- **Filament:** `AdminPanelProvider` (trackeado en Git), login `/admin/login`, overlay global (`->spa()` + hooks `STYLES_AFTER` / `BODY_START` / `SCRIPTS_BEFORE` / `SIDEBAR_FOOTER` / `BODY_END`). Branding Reputalis en todo el panel: `brandName`, color primario `#2ad4dc`, CSS `client-panel-theme` siempre (no solo rol cliente). Flags `.reputalis-theme-flags` + JS `syncReputalisClientPanel` en `livewire:navigated` / `pageshow`. Logo override `resources/views/vendor/filament-panels/components/logo.blade.php`. Navegación lateral del **rol cliente** (y preview staff en dashboard): `App\Support\ClientPanel`.
...
- **Reputación interna (rol cliente):** cabecera propia (título + subtítulo + pastillas de rango) y 4 KPI cards; las agujas ApexCharts de la primera fila quedan solo para superadmin/distribuidor. Nuevo rango `last_year` (12 meses) en `InternalReputationDateRange`. Cards: **Evolución de la satisfacción** (media acumulada ponderada; badge = media del periodo) | **Puntos de mejora** (barras mockup); debajo **Crecimiento acumulado** (opción B: total histórico + chip del incremento del rango) | **Operarios**. Sin pastillas internas Rango/Días/Horas. Granularidad: Hoy=hora, 7d=día, 30d=semana, 12m/Acumulado=mes. Detalle de operario y de punto de mejora reutilizan esas pastillas y `buildClientEvolutionOptions`. Un punto de mejora sin selecciones en el rango no pinta línea a 0%. `clientStyle` / `cumulative` van en el JSON de los gráficos. Superadmin/distribuidor no cambian.
...
- **Reputación externa (Google / Outscraper):** contenido en `ReputacionExterna`; el rol cliente entra por el ítem de menú y ve **8 KPI cards** (mockup); superadmin/distribuidor siguen usando agujas y las pestañas del Dashboard. Sync `external-reputation:sync` 3×/día; alertas 1★/2★ + anomalía `total_drop`; plan en `docs/PLAN_REPUTACION_OUTSCRAPER.md`.
