        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('panelMessage.sender.fullname')
                    ->label('Remitente')
                    ->formatStateUsing(fn ($state, PanelMessageRecipient $record) => $record->panelMessage?->sender?->getFilamentName() ?? '—')
                    ->placeholder('—'),
            ...
            ->emptyStateHeading('No hay notificaciones')
            ->emptyStateDescription('Cuando un distribuidor cree un cliente pendiente de activación, aparecerá aquí.')
