...
- **App\Filament\Resources\ClientResource**: Recurso Filament para gestionar clientes (`/admin`). Staff abre el **hub** `/{record}/inicio`; el propietario va al dashboard. Incluye ficha, encuesta (`PuntosDeMejora`), reputación externa, empleados, llamadas, soft deletes y permisos por rol. Subnavegación de registro vacía (`getRecordSubNavigation` = []).
...
- **App\Support\ClientPanel**: Detecta sesión de propietario o **preview staff** (`isActive` / `enterPreview` / sesión `reputalis.client_panel_preview_id`). Navegación lateral del cliente (grupos Panel principal / Gestión / Documentos). Decide barra de retorno (`showsStaffReturnBar`), ocultar sidebar (`hidesStaffSidebar`) y rutas de experiencia cliente vs cards staff. Resuelve el cliente de la ruta (`clientFromRoute`, también desde Livewire/`Employee`).
- **App\Http\Middleware\SyncClientPanelPreview**: Middleware de auth del panel: entra/mantiene preview en dashboard, cards staff, ficha/editar/empleados; sale de preview en el resto (salvo `livewire.*`).
- **App\Filament\Resources\ClientResource\Pages\ClientHub**: Landing staff al abrir un cliente (`/{record}/inicio`). El propietario se redirige al dashboard. Cards a Dashboard, Ficha, Encuesta, Empleados y Llamadas.
...
- **App\Filament\Pages\AdditionalTools**: Hub «Herramientas adicionales» (grupo Configuración). Visible a superadmin y distribuidor. Cards planas: Sectores (solo superadmin) e Imágenes de clientes.
...
- **App\Filament\Resources\ClientResource\Pages\EditClient**: Página de edición de cliente que sincroniza datos del propietario, controla la activación y fecha de expiración con un modal de confirmación y, al activar un cliente inactivo, envía un mensaje al distribuidor correspondiente. Cancelar (staff) vuelve a la **ficha**, no a `history.back()`. Tras guardar, staff va al hub.
